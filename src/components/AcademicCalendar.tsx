...
export default function AcademicCalendar({
  ...
  async function loadEvents() {
    ...
    try {
      ...
      // Auto-seed default Uttara University academic calendar if empty
      if (list.length === 0) {
        const seededList = await seedDefaultEvents();
  ...
  async function seedDefaultEvents(): Promise<AcademicEvent[]> {
    const today = new Date();
    
    // Create dates relative to today for realistic countdowns
    const midStart = new Date(today.getTime() + 14 * 24 * 60 * 60 * 1000); // 14 days from today
    const midEnd = new Date(today.getTime() + 20 * 24 * 60 * 60 * 1000);
    
    const finalStart = new Date(today.getTime() + 55 * 24 * 60 * 60 * 1000); // 55 days from today
    const finalEnd = new Date(today.getTime() + 62 * 24 * 60 * 60 * 1000);

    const classTestStart = new Date(today.getTime() + 5 * 24 * 60 * 60 * 1000); // 5 days from today
    const regStart = new Date(today.getTime() + 35 * 24 * 60 * 60 * 1000);

    const defaults: Omit<AcademicEvent, 'id'>[] = [
      {
        title: 'Spring 2026 Mid-Term Examinations',
        type: 'Mid',
        departmentId: 'all',
        startDate: midStart.toISOString().split('T')[0],
        endDate: midEnd.toISOString().split('T')[0],
        description: 'Uttara University official Mid-Term exams for all departments. Clear your dues before exam hall ticket issuance.',
        createdAt: new Date().toISOString(),
        createdByUsername: 'Uttara University Exam Controller'
      },
      {
        title: 'CSE Department Class Test & Quiz Week',
        type: 'Class Test',
        departmentId: departments.find(d => d.code === 'CSE')?.id || 'all',
        startDate: classTestStart.toISOString().split('T')[0],
        description: 'Departmental continuous evaluation class tests for 1st & 2nd year CSE batches.',
        createdAt: new Date().toISOString(),
        createdByUsername: 'CSE Academic Coordinator'
      },
      {
        title: 'Summer 2026 Pre-Registration Window',
        type: 'Registration',
        departmentId: 'all',
        startDate: regStart.toISOString().split('T')[0],
        description: 'Course pre-registration for the upcoming Summer 2026 trimester via student portal.',
        createdAt: new Date().toISOString(),
        createdByUsername: 'Registrar Office'
      },
      {
        title: 'Spring 2026 Final Examinations',
        type: 'Final',
        departmentId: 'all',
        startDate: finalStart.toISOString().split('T')[0],
        endDate: finalEnd.toISOString().split('T')[0],
        description: 'Comprehensive trimester final examinations across all faculties and departments.',
        createdAt: new Date().toISOString(),
        createdByUsername: 'Uttara University Exam Controller'
      }
    ];

    const seeded: AcademicEvent[] = [];
    for (const item of defaults) {
      try {
        const docRef = await addDoc(collection(db, 'academic_events'), item);
        seeded.push({ id: docRef.id, ...item });
      } catch (e) {
        console.error('Failed to seed academic event:', e);
      }
    }
    return seeded;
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-in fade-in duration-300">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-indigo-900 via-indigo-950 to-slate-950 rounded-3xl p-6 sm:p-10 text-white shadow-2xl border border-indigo-800/40">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
          ...
          {/* Admin / Moderator Add Action */}
          {currentUser && (currentUser.role === 'super_admin' || currentUser.role === 'moderator') && (
            <button
              onClick={() => setAddModalOpen(true)}
              className="shrink-0 px-5 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-extrabold rounded-2xl shadow-xl shadow-indigo-600/30 transition-all flex items-center justify-center gap-2 text-xs uppercase tracking-wider"
            >
