...
export default function QuestionDetail({ 
  ...
  // Handle Question Deletion with Automatic ImgBB Cleanup
  async function handleConfirmDeleteQuestion() {
    if (!question) return;
    setIsDeletingQuestion(true);

    try {
      const res = await deleteQuestionWithImages(question);
      if (!res.success) {
        throw new Error(res.error || 'Failed to delete question paper.');
      }
      
      showNotif('success', 'Question paper and its images from ImgBB deleted successfully.');
      ...
      console.error(err);
      showNotif('error', err.message || 'Failed to delete question paper.');
  ...
  const rawPages = (question.imageUrls && question.imageUrls.length > 0) 
    ? question.imageUrls.filter(Boolean)
    : (question.imageUrl ? [question.imageUrl] : []);
  
  const pages = rawPages.map((url) => formatImageUrl(url));
  const currentImgUrl = pages[activeImageIndex] || (pages[0] || 'https://placehold.co/800x1200/1e293b/ffffff?text=No+Image+Available');
    ...
    <div id="question-detail-root" className="w-full max-w-7xl mx-auto py-8 px-4 sm:px-6">
          ...
          <div className="bg-white dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-900 p-4 sm:p-6 shadow-sm">
            ...
            >
              <img 
                ...
                onError={(e) => handleImageError(e, `${question.courseCode || 'UU'} Question Paper`)}
              />
              
              {/* Overlay auto-zoom indicator badge */}
              <div className="absolute bottom-3 right-3 bg-slate-950/80 backdrop-blur-sm px-2.5 py-1 text-[10px] font-bold text-slate-300 rounded-lg pointer-events-none select-none transition-opacity duration-200 opacity-85 group-hover:opacity-0 shadow-md border border-white/5 flex items-center gap-1.5">
                <ZoomIn className="w-3.5 h-3.5 text-slate-400" />
                <span>Hover paper to auto zoom</span>
              </div>
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-slate-950 p-6 rounded-2xl border border-slate-200 dark:border-slate-900 shadow-sm space-y-6">
            ...
            <div className="space-y-2 pt-2">
              ...
              {/* Edit Question Paper Button (for Admins/Moderators or original Uploader) */}
              {(currentUser?.role === 'super_admin' || currentUser?.role === 'moderator' || currentUser?.uid === question.uploadedByUID) && (
                <div className="space-y-2">
                  <button
                    onClick={() => setIsEditModalOpen(true)}
                    className="w-full py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 text-slate-800 dark:text-slate-200 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-colors border border-slate-200/50 dark:border-slate-800 cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="w-full py-2 bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/40 dark:hover:bg-rose-900/50 text-rose-600 dark:text-rose-400 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors border border-rose-200/50 dark:border-rose-900/50 cursor-pointer"
                  >
      ...
      <EditQuestionModal
        ...
        onSuccess={() => {
          ...
          showNotif('success', 'Question paper details updated successfully!');
        }}
      />
      ...
      {showDeleteModal && question && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-md bg-white dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl p-6 space-y-4">
            <div className="flex items-center gap-3 text-rose-600">
              <div className="p-2.5 bg-rose-50 dark:bg-rose-950/50 rounded-xl border border-rose-200 dark:border-rose-900">
                <Trash2 className="w-5 h-5" />
              </div>
              <div>
                <h3 className="text-base font-bold font-display text-slate-900 dark:text-white">Delete Question Paper</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">Permanent action</p>
              </div>
            </div>

            <div className="p-3 bg-slate-50 dark:bg-slate-900/60 rounded-xl border border-slate-200/60 dark:border-slate-800 text-xs space-y-1.5">
              <p className="font-semibold text-slate-800 dark:text-slate-200">{question.courseCode} - {question.courseName}</p>
              <p className="text-[11px] text-slate-500 dark:text-slate-400">Exam: {question.examType}</p>
              <div className="pt-2 border-t border-slate-200 dark:border-slate-800 flex items-center gap-1.5 text-amber-600 dark:text-amber-400">
                <AlertTriangle className="w-4 h-4 shrink-0" />
                <span>All uploaded image pages will automatically be deleted from ImgBB as well.</span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              <button
                ...
                disabled={isDeletingQuestion}
                className="px-4 py-2 bg-rose-600 hover:bg-rose-700 disabled:opacity-50 text-white text-xs font-bold rounded-xl flex items-center gap-2 shadow-md transition-all cursor-pointer"
              >
                {isDeletingQuestion ? (
                  <>
                    <Loader2 className="w-3.5 h-3.5 animate-spin" />
                    <span>Deleting from Database & ImgBB...</span>
                  </>
                ) : (
                  <>
                    <Trash2 className="w-3.5 h-3.5" />
                    <span>Delete Paper & Images</span>
                  </>
                )}
              </button>
            </div>
