...
export default function Dashboard({ 
  ...
  return (
    <div id="dashboard-root" className="w-full max-w-7xl mx-auto py-8 px-4 sm:px-6">
      ...
      {(() => {
        const title = upcomingEvent?.title || 'Spring 2026 Mid-Term Examinations';
        const typeLabel = upcomingEvent?.type || 'Mid';
        const startDate = upcomingEvent?.startDate || '2026-08-10';
        const endDate = upcomingEvent?.endDate;
        const countdown = getEventCountdown(startDate);

        return (
          <div className="relative overflow-hidden bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 border-2 border-indigo-500/40 rounded-3xl p-6 sm:p-7 shadow-2xl mb-8 text-white">
            ...
            <div className="absolute bottom-0 left-1/4 -mb-10 w-48 h-48 bg-amber-500/10 rounded-full blur-2xl pointer-events-none"></div>

            <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              
              {/* Left Column: Big Bold Semester Exam Term Title & Badge */}
              <div className="space-y-2.5 max-w-xl">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="px-3 py-1 bg-amber-500/20 text-amber-300 border border-amber-400/30 text-xs font-black uppercase tracking-widest rounded-full flex items-center gap-1.5 shadow-sm">
                ...
                <p className="text-xs sm:text-sm text-indigo-200/90 font-medium flex items-center gap-2">
                  ...
                  <span>
                    Exam Schedule: <strong className="text-white font-bold">{startDate}</strong> {endDate ? `to ${endDate}` : ''}
                  </span>
                </p>
              </div>

              {/* Right Column: Live Days Countdown Ticker & Action Button */}
              <div className="flex flex-col sm:flex-row items-center gap-5 shrink-0 pt-2 lg:pt-0 border-t lg:border-t-0 border-indigo-800/60">
                
                {/* Live Digital Countdown Display */}
                <div className="flex items-center gap-2 sm:gap-3 bg-slate-950/80 p-3 rounded-2xl border border-indigo-500/30 shadow-inner">
                  <div className="flex flex-col items-center justify-center w-14 sm:w-16 h-14 sm:h-16 bg-slate-900 rounded-xl border border-indigo-500/30">
                    <span className="text-lg sm:text-2xl font-black font-mono text-amber-400">
                      {String(countdown.days).padStart(2, '0')}
                    </span>
                    <span className="text-[9px] font-bold text-indigo-300 uppercase">Days</span>
                  </div>
                  ...
                  <div className="flex flex-col items-center justify-center w-14 sm:w-16 h-14 sm:h-16 bg-slate-900 rounded-xl border border-indigo-500/30">
                    ...
                    <span className="text-[9px] font-bold text-indigo-300 uppercase">Hours</span>
                  </div>
                  <span className="text-lg font-extrabold text-indigo-500">:</span>
                  <div className="flex flex-col items-center justify-center w-14 sm:w-16 h-14 sm:h-16 bg-slate-900 rounded-xl border border-indigo-500/30">
                    <span className="text-lg sm:text-2xl font-black font-mono text-white">
                    ...
                    <span className="text-[9px] font-bold text-indigo-300 uppercase">Mins</span>
                  </div>
                  <span className="text-lg font-extrabold text-indigo-500">:</span>
                  <div className="flex flex-col items-center justify-center w-14 sm:w-16 h-14 sm:h-16 bg-indigo-600 rounded-xl border border-indigo-400/40 shadow-md">
                    <span className="text-lg sm:text-2xl font-black font-mono text-white animate-pulse">
                      {String(countdown.seconds).padStart(2, '0')}
                    </span>
                    <span className="text-[9px] font-bold text-indigo-100 uppercase">Secs</span>
                  </div>
                </div>
                ...
                {onNavigateToCalendar && (
                  <button
                    ...
                    className="w-full sm:w-auto px-5 py-3 bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-500 hover:to-indigo-400 text-white font-extrabold rounded-2xl shadow-xl shadow-indigo-600/30 transition-all flex items-center justify-center gap-2 text-xs uppercase tracking-wider shrink-0"
                  >
