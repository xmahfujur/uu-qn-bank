...
export default function Layout({ 
  ...
  React.useEffect(() => {
    getRedirectResult(auth)
      ...
      .catch((err: any) => {
        console.error('Redirect sign-in result error details:', {
          code: err?.code,
          message: err?.message,
          error: err
        });
  ...
  async function handleLoginWithRedirect() {
    ...
    try {
      ...
      console.error('Direct Google Auth Redirect error details:', {
        code: err?.code,
        message: err?.message,
        error: err
      });
      setAuthError(`Sign-in failed (${err?.code || 'redirect-error'}): ${err?.message || 'Please try again.'}`);
      setIsLoggingIn(false);
    }
  }

  async function handleLogin() {
    // CRITICAL: Call signInWithPopup IMMEDIATELY and SYNCHRONOUSLY inside the click event handler
    // to preserve browser user activation (prevents auth/popup-blocked on Desktop Chrome and Edge).
    let popupPromise: Promise<import('firebase/auth').UserCredential>;
    try {
      popupPromise = signInWithPopup(auth, googleProvider);
    } catch (syncErr: any) {
      console.error('Synchronous popup invocation failed:', {
        code: syncErr?.code,
        message: syncErr?.message,
        error: syncErr
      });
      // Fallback immediately to redirect
      return handleLoginWithRedirect();
    }
    ...
    try {
      ...
      // Print comprehensive error telemetry (error.code, error.message, full error object)
      console.error('Google Auth Popup Login error details:', {
        code: err?.code,
        message: err?.message,
        error: err
      });

      // User closed popup explicitly
      if (
        err?.code === 'auth/popup-closed-by-user' || 
        err?.code === 'auth/user-cancelled'
      ) {
      ...
      // Automatic Fallback to Redirect on popup blocked, cancelled, or iframe restriction
      console.warn('Popup login failed or blocked. Automatically retrying with signInWithRedirect()...', {
        code: err?.code,
        message: err?.message,
        error: err
      });
      ...
      try {
        ...
        console.error('Redirect auth fallback error details:', {
        ...
        if (err?.code === 'auth/unauthorized-domain') {
          ...
          setAuthError(`Sign-in error (${err?.code || 'popup-blocked'}): ${err?.message || 'Please use direct sign-in below.'}`);
        }
      }
    } finally {
      setIsLoggingIn(false);
            ...
            <div className="flex-1 text-xs">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                <button
                  onClick={handleLogin}
                  disabled={isLoggingIn}
                  className="px-3 py-1.5 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-xl font-semibold transition-all flex items-center gap-1 text-xs disabled:opacity-50 cursor-pointer"
                >
                  <span>Try Popup Again</span>
                </button>
              </div>
            </div>
            <button 
