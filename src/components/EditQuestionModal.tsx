...
export default function EditQuestionModal({ isOpen, onClose, question, currentUser, onSuccess }: EditQuestionModalProps) {
  ...
  useEffect(() => {
    if (question && isOpen) {
      ...
      setDeleteUrls(question.deleteUrls && Array.isArray(question.deleteUrls) ? question.deleteUrls : []);
    ...
    try {
      ...
      if (currentUser) {
        await recordAuditLog({
          ...
          category: 'Papers',
          user: currentUser,
          details: `Updated question paper details & metadata for "${courseCode.trim()} - ${courseName.trim()}".`,
          targetId: question.id,
          targetName: `${courseCode.trim()} - ${courseName.trim()}`,
          metadata: {
            courseCode: courseCode.trim(),
            courseName: courseName.trim(),
            examType,
            imagesCount: filteredImageUrls.length
          }
        });
      }
          ...
          <div className="p-4 bg-slate-50 dark:bg-slate-900/40 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-slate-500">Upload single or multiple pages for this question paper</p>
              </div>
              {imageUrls.length > 0 && (
            ...
            {imageUrls.length > 0 && (
              <div className="space-y-3 pt-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-slate-500">Verify image visibility before saving</span>
                </div>
