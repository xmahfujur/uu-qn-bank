...
export default function UploadModal({ isOpen, onClose, currentUser, onUploadSuccess }: UploadModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!selectedDept || !selectedBatch || !selectedSemester || !courseCode || !courseName || filteredImageUrls.length === 0) {
    ...
    try {
      ...
      setNotification({ type: 'success', message: 'Question paper uploaded successfully!' });
      setTimeout(() => {
        onUploadSuccess();
        resetForm();
        onClose();
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-slate-500">Add 1 or more pages (direct links like ImgBB, Cloudinary, etc.)</span>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Course Code *</label>
              <input
