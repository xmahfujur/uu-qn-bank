  ...
  async function uploadFilesToImgBB(files: FileList | File[]) {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        if (resData && resData.success && resData.data && (resData.data.url || resData.data.display_url || resData.data.image?.url)) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm overflow-y-auto">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div id="imgbb-direct-upload" className="p-4 bg-slate-50 dark:bg-slate-900/40 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 space-y-4">
            ...
            <div className="block sm:hidden bg-gradient-to-r from-indigo-900 via-indigo-800 to-purple-900 text-white rounded-2xl p-4 shadow-md border border-indigo-500/30">
              <div className="flex items-start gap-3">
                <div className="p-2.5 bg-amber-400 text-slate-950 rounded-xl shrink-0 shadow-sm mt-0.5">
                  <Camera className="w-5 h-5" />
                </div>
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <h4 className="text-xs font-black uppercase tracking-wider text-amber-300">Mobile Camera Scanner</h4>
                    <span className="px-2 py-0.5 bg-white/10 text-white text-[9px] font-bold rounded-full border border-white/20">
                      Mobile Feature
                    </span>
                  </div>
                  <p className="text-[11px] text-indigo-100 mt-1 leading-snug">
                    Take photos of exam pages one by one with your phone camera, review clarity, retake blurry images, and upload directly!
                  </p>
      ...
      {croppingPageIndex !== null && capturedPages[croppingPageIndex] && (
        <div className="fixed inset-0 z-[140] bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh]">
            ...
            <div className="p-4 bg-slate-900 text-white flex items-center justify-between border-b border-slate-800">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-sm font-black text-white">Crop & Adjust Page {croppingPageIndex + 1}</h3>
                  <p className="text-[11px] text-slate-400">Drag box handles for free size crop or select a box aspect ratio</p>
                </div>
              </div>
              <button
                type="button"
                onClick={() => setCroppingPageIndex(null)}
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 pt-1 border-t border-slate-200 dark:border-slate-800">
                <div className="flex items-center gap-1.5">
                  <span className="text-[11px] font-bold text-slate-600 dark:text-slate-400 shrink-0">Rotate:</span>
                  <button
                    type="button"
                    onClick={() => setRotation(r => (r - 90) % 360)}
                    className="p-2 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-lg text-xs font-bold transition-colors cursor-pointer"
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[11px] font-bold text-slate-600 dark:text-slate-400 shrink-0">Zoom:</span>
                  <input
                    type="range"
                    min={1}
                    max={3}
                  ...
                  />
                  <span className="text-[10px] font-mono font-bold text-slate-500 w-6 text-right">{zoom.toFixed(1)}x</span>
                </div>
              </div>
