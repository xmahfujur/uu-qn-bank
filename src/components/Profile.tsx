...
export default function Profile({ currentUser, onNavigateToQuestion, onProfileUpdated }: ProfileProps) {
  ...
  const [questionToDelete, setQuestionToDelete] = useState<QuestionPaper | null>(null);
  ...
  async function handleConfirmDeletePaper() {
    ...
    try {
      const res = await deleteQuestionWithImages(questionToDelete);
      if (!res.success) {
        throw new Error(res.error || 'Failed to delete question paper.');
      }
      
      setMyUploads(prev => prev.filter(q => q.id !== questionToDelete.id));
      setNotification({ 
        type: 'success', 
        message: 'Question paper and its images from ImgBB deleted successfully.' 
      });
      setTimeout(() => setNotification(null), 3500);
      setRefreshTrigger(prev => prev + 1);
    } catch (err: any) {
      console.error(err);
      setNotification({ type: 'error', message: err.message || 'Failed to delete question paper.' });
    } finally {
    ...
    <div id="profile-root" className="w-full max-w-7xl mx-auto py-8 px-4 sm:px-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      title="Delete question paper and remove images from ImgBB"
                    >
          ...
          {/* Delete Confirmation Modal with ImgBB notification */}
          {questionToDelete && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-fade-in">
              <div className="w-full max-w-md bg-white dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl p-6 space-y-4">
                ...
                <div className="p-3 bg-slate-50 dark:bg-slate-900/60 rounded-xl border border-slate-200/60 dark:border-slate-800 text-xs space-y-1.5">
                  <p className="font-semibold text-slate-800 dark:text-slate-200">{questionToDelete.courseCode} - {questionToDelete.courseName}</p>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400">Exam: {questionToDelete.examType}</p>
